...
| admcode | String | 선택 |  | 행정코드 |
| depth | String | 선택 |  | 하위시설물 요구 depth<br>1 : 1 depth 만 요청(최상위 depth)<br>2 : 2 depth 까지 요청<br>3 : 3 depth 까지 요청 _depth값이 설정되지 않은 경우 1로 설정<br>_ depth 설정 시 아래 Response 처럼 subpoi 상세 정보가 depth에 맞게 반환됨 |
