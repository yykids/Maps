...
> ※ TOAST Maps APIで使用される座標は、THINKWARE専用座標にのみ使用されます。
...
| ---------------------------------------- | ---------------------------------------- | ---------------- | ---------------------------------------- | ---------------------------------------- |
| TMWI.initMap(map_div_name, twX, twY, level, arrange_type, map_type) | map_div : String<br>	マップを入れるdivタグID    | initCB <br><br>  | マップ初期化成否<br>'true'：成功<br> 'false'：失敗 | マップを使用するために、最初に呼び出す必要がある初期化関数です。  |
...
| TMWI.getLevel()                          |                                          | getLevelCB       | マップの現在レベル                          | マップのレベルを取得します。                           |
| TMWI.getCenter()                         |                                          | getCenterCB      | マップの現在中心座標<br> 'twX&#124;twY'           | マップの中心座標を取得します。                         |
| TMWI.getExtent()                         |                                          | getExtentCB      | マップの領域座標<br> 'leftX&#124;topY&#124;rightX&#124;bottomY' | 現在マップが表示される領域座標を取得します。                 |
| TMWI.setMoveend()                        |                                          | moveendCB        | 拡大、縮小、移動後マップ中心座標とレベル <br>'twX&#124;twY&#124;level' | マップにmoveendイベントを登録します。<br>moveend：マップ拡大、縮小、移動が終わった時 |
| TMWI.removeMoveend()                     |                                          |                  |                                          | マップでmoveendイベントを削除します。                 |
| TMWI.setTouchend()                       |                                          | setTouchendCB    | タッチしたマップ座標<br> 'twX&#124;twY'             | マップにtouchendイベントを登録します。<br>  touchend：マップタッチが終わった時 |
| TMWI.removeTouchend()                    |                                          |                  |                                          | マップでtouchendイベントを削除します。                |
| TMWI.setZoomend()                        |                                          | zoomendCB        | 拡大、縮小後マップ中心座標とレベル<br> 'twX&#124;twY&#124;level' | マップにzoomendイベントを登録します。 <br> zoomend：マップ拡大、縮小が終わった時 |
| TMWI.removeZoomend()                     |                                          |                  |                                          | マップでzoomendイベントを削除します。                 |
| TMWI.setTouchEvent(event_name)           | event_name：String<br>	登録するイベント名<br> 'touchstart'：マップタッチを始めた時<br>  'touchend'：マップタッチが終わった時<br>  'longpress'：マップを長押しした時 | setTouchEventCB  | 発生したイベントとタッチしたマップ座標 <br>'event_name&#124;twX&#124;twY' | マップにtouch関連イベントを登録します。                  |
| TMWI.removeTouchEvent(event_name)        | event_name：String<br>	登録するイベント名<br> 'touchstart'：マップタッチを始めた時<br>  'touchend'：マップタッチが終わった時<br>  'longpress'：マップを長押しした時 |                  |                                          | マップでtouch関連イベントを削除します。                 |
...
|                                          | iconWidth : Number <br>Markerイメージの幅  |                  |                                          |                                          |
